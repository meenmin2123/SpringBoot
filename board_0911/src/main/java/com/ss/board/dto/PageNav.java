...
public class PageNav {
    ...
    private int totalPage; // 총 페이지 건수
    private int firstPage; // 페이지 리스트에서 첫 페이지
    private int lastPage; // 페이지 리스트에서 마지막 페이지


    public PageNav(int currentPage, int listCount, int listLimit) {
        super();
    ...
    public void pageSetting(int totalRowCount) {
        ...
        totalPage = (int) Math.ceil((double) totalRowCount / listLimit);
        System.out.println("totalPageCount : " + totalPage);
