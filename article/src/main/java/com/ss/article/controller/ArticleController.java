...
//		return "articles/new";
	...
	public String delete(RedirectAttributes redirectAttributes, @PathVariable Long id) {
		log.info("delete() 실행");
		Article target = articleSql.findById(id).orElse(null);
		...
		if(target != null) {
			try {
	            ...
	            log.error("삭제 오류", e);
	            redirectAttributes.addFlashAttribute("msg", "삭제 중 문제가 발생했습니다.");
	        }
		} else {
			redirectAttributes.addFlashAttribute("msg", "삭제할 데이터를 찾을 수 없습니다."); 
		}
